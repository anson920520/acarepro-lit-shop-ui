...
<template>
	<view class="allOrder"  @click="$store.commit('setState', { key: 'showBox',value:false })">
		...
		<view class="allOrderTop">
			...
			<view class="myTitle ju" v-else>我的订单</view>
		</view>
			...
			<view class="orderList" v-for="(item, i) in sliceData" :key='i' :id="'order' + i" @click="longtab(i)">
				<view class="orderListTop">
					...
					<view class="clearBoth">
						...
						<view class="width50">
							<text>返利总金额: ￥{{item.rebate_total}}</text>
						</view>
						<view class="width50">
							<text>包邮: </text>
			...
			del (item) {
				...
				uni.showModal({
					...
					success (res) {
						if (res.confirm) {
							uni.request({
								url: that.$store.state.baseURL + "order/delete/" + item.ID,
								method:"DELETE",
								success (res) {
									console.log(res)
				...
				tempFilePaths.forEach((item,i) => {
					...
					img.onload = function() {
						...
						EXIF.getData(img, function () {  
							 ...
							 // console.log(base64);
							 uni.uploadFile({
								 url: _this.$store.state.baseURL + "order/lock/" + order.ID, //仅为示例，非真实的接口地址
								 filePath: base64,
								 name: "file",
								 fileType: 'image',
								 formData: {
